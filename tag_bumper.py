...
def main() -> int:
    ...
    for repository in all_repositories:
        try:
            ...
            semver_tags = tags_to_semantic_versions(source_repo_object)
            main_commit = get_main_commit(repo=source_repo_object)
            tag_commit = get_tag_commit(repo=source_repo_object, tag_name=str(latest_version(semver_tags)))
            if main_commit == tag_commit:
                outcomes.append(f"{repository.name}: Commits on main and {latest_version(semver_tags)} match, no action will be taken!")
                continue
            else:
                outcomes.append(f"{repository.name}: needs main tag updated from {tag_commit} to {main_commit} (tag version {latest_version(semver_tags)})")
                move_tag_forward(repo=source_repo_object, tag_name=str(latest_version(semver_tags)))
